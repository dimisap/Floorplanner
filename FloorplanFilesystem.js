...
FloorplanFilesystem.prototype.saveFloorplan = function () {
    if (checkLocalStorage()) {
        ...
        if (saveName === this.UNSAVED_FILENAME || saveName == null || saveName == undefined) {
			...
			//alert(this.floorplan.model.toJson());
			// Append the JS variables as query string
			sessionStorage.setItem("floorplan", this.floorplan.model.toJson());

			var src = "default.php";
			src += "?fileid=" + this.getCurrentFileName();
			//src += "&floorplan=" + this.floorplan.model.toJson();
			//alert(document.getElementById("floorplan").innerHTML);
			...
			$.ajax({
			   ...
			   url : 'updateMySql.php', // your php file
			   type : 'POST', // type of the HTTP request
			   data: 'fileid=' + this.getCurrentFileName() + '&floorplan=' + this.floorplan.model.toJson(),
...
// Load floorplan initially
FloorplanFilesystem.prototype.loadFloorplanHome = function () {
	var temp = 'adsdadasd';
	console.log(temp); 
    //var floorplan = this.floorplan;
    //var listbox = document.getElementById(this.state.filesToOpenListId);
    var fileName = document.getElementById("floorplan").innerHTML; // get selected filename
    ...
    if (fileName !== undefined) {
		...
		var data = makeAjaxCall(fileName);
...
function makeAjaxCall (fileName) {
		var temp = "123456789";
		//alert(temp);
		$.ajax({
			
		   url : 'getFloorPlan.php', // your php file
		   type : 'GET', // type of the HTTP request
		   data: 'fileName=' + fileName,
		   success : function(data){
			  document.getElementById("floorplantext").innerHTML=data; 
    ...
    if (fileName !== undefined) {
		...
		src += "?fileid=" + this.getCurrentFileName();
				...
				$.ajax({
				   ...
				   url : 'deleteMySql.php', // your php file
				   type : 'POST', // type of the HTTP request
				   data: 'fileid=' + this.getCurrentFileName() + '&floorplan=' + this.floorplan.model.toJson(),
				   success : function(data){
